...
console.log(F.wrap);
			...
			if (current.closeBtn) {
				...
				$(current.tpl.closeBtn).appendTo($('.fancybox-overlay')).bind( isTouch ? 'touchstart.fb' : 'click.fb', function(e) {
					e.preventDefault();
			...
			if (current.arrows && F.group.length > 1) {
				if (current.loop || current.index > 0) {
					$(current.tpl.prev).appendTo($('.btn-cont')).bind('click.fb', F.prev);
				}

				if (current.loop || current.index < F.group.length - 1) {
					$(current.tpl.next).appendTo($('.btn-cont')).bind('click.fb', F.next);
				}
			}
